...
int Gal2Eq(double b, double l, double *RA, double *dec)
{
	double sind, l1, l2, l3, tn, td;
   ...
   tn = cos(b)*sin(l-l1)*cos(l2) - sin(b)*sin(l2);
   td = cos(b)*cos(l-l1);
   *RA = (atan2(tn,td) + l3)*180.0/PI;

   sind = cos(b)*sin(l-l1)*sin(l2) + sin(b)*cos(l2);
   *dec = asin(sind)*180.0/PI;
}
...
void ConvertToDDMMSS(double X, char *DDMMSS, int LeadingSign)
{
      ...
      {
            if (sign < 0.0)
                  sprintf(DDMMSS,"-%03d%02d%02d",iD,iM,iS);
            else
                  sprintf(DDMMSS,"+%03d%02d%02d",iD,iM,iS);
      }
      else
            sprintf(DDMMSS,"%02d%02d%02d",iD,iM,iS);
}
